...
public class FiltroGeneralizado {
	 ...
	 * Principal.
	 *
	 * @param imagen --> este parametro se refiere a la imagen que se va a ser procesada
	 * @return retorna un BufferedImage
	 */
	public static BufferedImage principal(BufferedImage imagen) {
		Mat m = bufToMat(imagen);   //convertimos bufferedImage a un objeto tipo Mat
		double [][][] img = getMatrix(m); //Obtenemos la matriz de pixeles de la imagen
		double [][][] filtermatrix = filter(5,2,img); //aplicamos el filtro a la matriz de pixeles
		Mat withfilter = getFixedImage(filtermatrix); //se convierte de double[][][] a Mat
		BufferedImage buffer = mat2BufferedImage(withfilter) ; //convierte de Mat a BufferedImage
		return buffer;
	}
	 ...
	 * Gets the kernel.
	 *
	 * @param k --> tamaño de la ventana del Kernel
				...
				for(int y=1; y < U[0].length-1; y++){
					...
					for(int i=xkmin; i < xkmax+1; i++){			// se desliza F sobre U
						for(int j=ylmin; j < ylmax+1; j++){
						   result += U[x][y] * gaussKernel[i%gaussKernel.length][j%gaussKernel.length];
						}
					}
					Ures[x][y] = result;
				}
